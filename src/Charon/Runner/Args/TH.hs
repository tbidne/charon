    ...
    qs =
      GRT.firstSuccessQ
        [ GRT.embedGitError gitDataFromGitQ,
          GRT.runGitInEnvDirQ [osstr|CHARON_HOME|] gitDataFromGitQ,
          gitDataFromEnvQ
        ]
...
--
-- - CHARON_MODIFIED: unix time like "1746055623"
-- - CHARON_HASH: long hash
-- - CHARON_SHORT_HASH: short hash
--
-- We have to convert the unix time into the intended format
-- YYYY-MM-DD.
gitDataFromEnvQ :: Q (Either GitRevError (OsString, OsString, OsString))
gitDataFromEnvQ = do
  let dateVar = [osstr|CHARON_MODIFIED|]
  d <- GRT.embedEnvError $ fmap (>>= displayUnixTime dateVar) (GRT.envValQ dateVar)
  h <- validateHash 40 <$> GRT.envValQ [osstr|CHARON_HASH|]
  sh <- validateHash 7 <$> GRT.envValQ [osstr|CHARON_SHORT_HASH|]
  pure $ liftA3 (,,) d h sh
  where
