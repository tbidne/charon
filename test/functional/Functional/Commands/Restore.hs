  ...
  where
    runner = do
      ...
      usingReaderT testEnv $ appendTestDirM "restoreUnknownError" $ do
        ...
        exBs <- captureCharonExceptionTermBS @TrashEntryNotFoundE testDir restoreArgList
  ...
  where
    runner = do
      ...
      usingReaderT testEnv $ appendTestDirM "restoreCollisionError" $ do
        ...
        -- RESTORE
        restoreArgList <- withSrArgsM ["restore", "--no-prompt", "f1"]
        exBs <- captureCharonExceptionTermBS @RestoreCollisionE testDir restoreArgList
