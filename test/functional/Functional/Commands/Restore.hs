  ...
  where
    runner = do
      ...
      usingReaderT testEnv $ appendTestDirM "savesCompletions" $ do
        ...
        permDelArgList <- withSrArgsM ["restore", "--prompt", "off", "2", "4"]
        ...
        -- completions assertions
        restoreArgList <- readFileUtf8ThrowM (xdgCharonState </> [osp|completions.txt|])
        let bs4 = completionsToBs restoreArgList
